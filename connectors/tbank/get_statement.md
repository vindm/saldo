...
Generate and download a bank statement for a client's account in T-Bank Business for a period, save it to `_Inbox/`, parse Dr/Cr turnover. Replaces waiting for the statement from the client for the direct contour.
...
|---|---|---|
| `client_id` | string | **required** (`client_d` / `client_e` / `client_f` / `client_g` …) |
...
- Claude in Chrome is authenticated in T-Bank (`business.tbank.ru`). On the first entry from the MCP tab a login wall is possible — see memory `mcp_chrome_cookie_isolation` (ask the operator to open T-Bank once in a regular tab, then retry).
...
2. The companies there are real button-cards (`button[automation-id=single-resource-large-card]`), click deterministically. Click the required client's card (Client A=client_d, Client A=client_e, Client A=client_f, Client A=client_g).
...
5. When the work is finished, restore the active company to the one the operator had (default — Client A (`client_d`)), the same way. **And close the MCP tabs I opened** (`tabs_close_mcp`) — don't leave junk behind (memory `close_browser_tabs_after_use`).
...
3. Move it to `output_folder` (canonical download pipeline — as in memory `downloads_mount_access_pattern`: access Downloads via direct path).
...
3. **Ignore the closing balance** (memory `bank_balances_not_our_zone`) — only the period's turnover matters.
4. For typical postings, check against memory `typical-postings-bank-statements` (utilities→60, SBP to own card→84, bookkeeping services from [redacted]→60).
...
- The file stays in `_Inbox/` for posting (the operator moves it into `SP <Surname>/`).
...
- For Client A (`client_d`) (AUSN): the statement = reconciliation with what the bank has already reported to the FTS, **not** the basis of the KUDIR.
