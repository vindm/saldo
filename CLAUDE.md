...
- **The sweep is the proactive half.** Reactive `mm_update` only touches a task when a signal arrives *for it*; `connectors/resolution_sweep/SKILL.md` re-checks the nearest active tasks on a schedule and advances those that became doable with no fresh signal — the missing sync moment. It **folds in `question_resolver`** (the open-question rung logic, no longer a separate scheduled job — open questions are first-class, not a side channel) and never closes a work thread or sends. One program, two triggers — chat (reactive) and the sweep (proactive) — both fanning out along `refs` to every related task.
...
> collectors (news, documents[cloud+local], email[multi-account], tg/whatsapp/max[by-chat], practice_management) → `resolution_sweep` (residue only — runs AFTER collectors, BEFORE monitors; proactive edge re-check + actualize over all active tasks, incl. open questions via the folded-in question_resolver rung logic) → monitors (deadline / staleness / threshold·weekly / counterparty·monthly) → `dashboards` (unconditional render).
